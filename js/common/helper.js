...
app.helper = (function() {
    var cbmanager = {
        ...
        clone: function(position) {
            var i, obj, clone, center, cloned = [],
                fieldlist = app.options.types.objResFields.concat(["prior", "next"]),
                that = this;
            if (this._list.length == 0 || typeof app.model.objects[this._list[0]] == 'undefined')
                return;
            center = {
                    ...
                    if (obj[fieldlist[field]] !== undefined) {
                        $.each(obj[fieldlist[field]].objects, function() {
                            if ($.inArray(this.obj, that._list) > -1) {
                                app.model.objects[cloned[i]][fieldlist[field]].objects.push($.extend(true, {}, this, {
            ...
            app.events.add("newObject", "clipboardCopy");
            return this;
        }
    }
    return {
        ...
        loadContent: function(selector) {
            var $content = $($.trim($(selector).html()));
        ...
        exportimg: function() {
            ...
            copy.setAttribute("width", Math.max(220, bb.width));
            copy.setAttribute("height", Math.max(220, bb.height));
            copy.setAttribute("style", "background:white;");
            // copy.removeChild(copy.getElementById("bpmGrid"));
            copy.removeChild(copy.getElementById("counter"));
            // copy.getElementById("bpm-figures").setAttribute("transform", "translate(-" + Math.max(0, bb.x) + ".5,-" + Math.max(0, bb.y) + ".5)");
            copy.getElementById("bpm-figures").setAttribute("transform", "translate(-" + Math.max(0, parseInt(bb.x) + 0.5) + ",-" + Math.max(0, parseInt(bb.y) + 0.5) + ")");
            return copy
        },
        ...
        postImage: function(opts) {
            ...
            var callObj = $.extend(true, {}, DEFAULT_CALL_OPTS, opts.call);
        ...
        lastOpenedmodel: function(fileId, cansave) {
            ...
            app.options.settings.session.currentFile = {
                storage: app.options.settings.General.storage,
                file: cansave === true ? fileId : null
            };
            app.options.save();
        },
        exportImgCapability: function() {
            return window.navigator.msSaveBlob ? false : true;
        },
        indexableText: function(model) {
            var a = [app.model.info.name];
            Object.keys(model.objects).forEach(function(key) {
                if (model.objects[key]["name"]) {
                    const name = model.objects[key].name.trim();
