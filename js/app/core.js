...
app.core = (function() {
	var removeObj = function(obj) {
		...
		if (obj.objType === app.options.types.objType.bpObject) {
			...
			if (obj.objClass == app.options.types.objClass.bpGenerator) {
				$.each($.map(app.model.objects, function(mobj) {
					return mobj.objClass == app.options.types.objClass.bpCheckPoint && mobj.allowFilter.indexOf(obj.id) != -1 ? mobj.id : null
				}), function(index, value) {
					app.model.objects[value].allowFilter.splice(app.model.objects[value].allowFilter.indexOf(obj.id), 1)
				})
			}
	...
	return {
	  ...
	  createObject : function(objClass, options) {
		  if (typeof app.model.objects == 'undefined') {
			  try {
				  throw new Error('Core objects not found');
			  } catch (err) {
