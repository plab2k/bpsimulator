...
(function($) {
    ...
    $.widget("ui.bpsDashboard", $.ui.bpsdialog, {
        _create: function() {
            var that = this;
            $(".fbut", this.element).on("click", function() {
                that._trigger("exportImage", null, document.getElementById("svgdashboard"));
                return false;
            });
            this._super();
        },
        _destroy: function() {
            $(".fbut", this.element).off();
            this._super();
        },
        ...
        _updateGauge: function() {
            var arr = app.simulation.collector.getwaitQLength();
            var qlen = arr.length == 0 ? 0 : arr[arr.length - 1].count,
                qwork = app.simulation.collector.getWorkQlen();
        ...
        _updateTopRight: function() {
            var o = app.simulation.collector.getTaskStat(),
                sumgrad = 0,
                sum = o.processSum + o.deliverySum + o.waitingSum,
                grad, percent;
            if (o.count == 0) {
                return;
            }
        ...
        _updateSummary: function() {
            var i, sumcr = 0,
                sumfin = 0,
                arr = app.simulation.collector.getCrFin();
            for (i = 0; i < arr.length; i++) {
                if (arr[i].action === app.options.types.eventType.taskNew)
                    sumcr += arr[i].count
                else
