...
(function($) {
	...
	$.widget("ui.bpOutputer", $.ui.basedetail, {
	  ...
	  update : function() {
		  var content = "", item = "", template = app.helper.loadContent("#customerItem"), obj;
		  ...
		  $.each(this._obj().next.objects, function() {
			  ...
			  $("option[value=" + this.transferMult + "]", item).attr("selected", true);
			  $("input[name=allocationPercent]", item).attr("value", this.allocationPercent);
			  $(item).prepend("<h5>" + obj.name + "</h5>");
			  content += "<li data-objid=" + obj.id + ">" + item.html() + "</li>";
		  });
		  $(".nextList", this.element).append(content);
	...
	$.widget("ui.bpFunction", $.ui.bpOutputer, {
	  _create : function() {
		  ...
		  $("input[name=runTimeMin]", this.element).change(function() {
		  ...
		  $("input[name=runTimeMax]", this.element).change(function() {
	  ...
	  update : function() {
		  var content = "", obj;
		  var resTmpl = app.helper.loadContent("#resourceItem"), supTmpl = app.helper.loadContent("#supportItem");
		  var updateRes = function(item) {
			  var obj = app.model.objects[item.obj];
			  if (!obj)
				  throw new Error("Obj detail not found, item:" + item + ", obj:" + item.obj);
	...
	$.widget("ui.bpCheckPoint", $.ui.basedetail, {
	  ...
	  _create : function() {
		  ...
		  $("button[data-text=Add]").click(function() {
