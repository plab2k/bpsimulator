...
app.simulation.init = (function() {
	...
	return {
	  setup : function(time) {
		  var i, r, beginners = app.helper.tree.beginners, len = beginners.length, fbresult, obj;
		  path = [];
		  taskId = 0;
		  rate = 86400;
		  for (i in app.model.objects) {
			  ...
			  if (obj.objClass == app.options.types.objClass.bpExecute) {
				  ...
				  obj._simulation.utilizationTime = obj.timeRanges.length == 0 ? 60 * 60 * 24 : $.map(obj.timeRanges, function(o) {
