		  ...
		  exportmodel : function() {
			  svgAsDataUri(app.helper.exportimg(), {}, function(uri) {
				  ...
				  image.onload = function() {
					  ...
					  var binStr = atob(data.split(',')[1]), len = binStr.length, arr = new Uint8Array(len);
					  for (var i = 0; i < len; i++) {
						  arr[i] = binStr.charCodeAt(i);
					  }
					  app.storage.local.saveImage(arr, app.model.info.name, function(result) {
		  ...
		  showdashboard : function() {
			  showDialog("#dashboard", {
				  exportImage : function(event, data) {
					  svgAsDataUri(data, {}, function(uri) {
						  var image = new Image();
						  image.onload = function() {
							  var canvas = document.createElement('canvas');
							  canvas.width = image.width;
		  ...
		  showlibrary : function() {
			  showDialog("#library", {
			    ...
			    onLibraryUpdate : function() {
				    app.storage.library.update(function() {
