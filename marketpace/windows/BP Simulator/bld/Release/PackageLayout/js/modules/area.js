...
if (SVGElement && SVGElement.prototype) {
    ...
    SVGElement.prototype.addClass = function (className) {
        if (!this.hasClass(className)) {
            this.setAttribute('class', (this.getAttribute('class')) ? this.getAttribute('class') + ' ' + className : className);
        }
    };
    ...
    return {
        ...
        figure: {
            add: function (options) {
                var width = app.options.settings.UI.objectWidth, radius = 5, indent = 10, d, figure, height = 40, txt, elemArr = [], title, group;
        ...
        polyline: {
            ...
            _isLong: null,
            _xor: function (x, y) {
                var txt, from = app.model.objects[this._from], to = app.model.objects[this._to];
            ...
            _testcross: function (p1, p2, p3, p4) {
                ...
                {
                    x = p1.X + ((p2.X - p1.X) * (p3.Y - p1.Y)) / (p2.Y - p1.Y);
            ...
            _correct: function () {
                var i, lp = null, x, y, seg = app.area.segments;
                ...
                for (i = 1; i < this._points.length; i++) {
                    if (seg && this._points[i].x == this._points[i - 1].x && seg.ver[this._points[i].x]) {// vertical segment
                        var from = this._points[i - 1].y, to = this._points[i].y;
                        ...
                        var from = this._points[i - 1].x, to = this._points[i].x;
                ...
                // add arc
                var path = "M" + this._points[0].x + "," + this._points[0].y, radius, dir, v;
                for (i = 1; i < this._points.length; i++) {
                    // find crossing
